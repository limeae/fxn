...
#with storage fxn:array cmd {group, i}

say macro/book
#save waypoint data to matches
$data modify storage waypoint:matches merge set from storage waypoint:$(group) list[$(i)]
#save group to matches
$data modify storage waypoint:matches merge.group set value $(group)

#get highest numbered page
execute store result storage waypoint:matches merge.page int 1 run function fxn:array/sizeof {path:'storage waypoint:matches pages'}
