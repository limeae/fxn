...
#(if match) clone with timedelta
$execute if score result ftemp matches 1.. run function tool:entry/cmd {macros:{merge:{timedelta:$(timedelta)}}, command:'function tool:math/merge'}
execute if score result ftemp matches 1.. run return run function tool:entry/cmd {macros:{command:"function tool:op/macro/interpolate"}, command:"function tool:entry/data"}
