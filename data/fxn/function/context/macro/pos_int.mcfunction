...
$execute if data storage fxn:context {pos:{x:1}} run \
    execute store result $(path).x int 1 run data get entity @s Pos[0]
$execute if data storage fxn:context {pos:{y:1}} run \
    execute store result $(path).y int 1 run data get entity @s Pos[1]
$execute if data storage fxn:context {pos:{z:1}} run \
    execute store result $(path).z int 1 run data get entity @s Pos[2]
#rot r0,r1
$execute if data storage fxn:context {pos:{r0:1}} run \
    execute store result $(path).r0 int 1 run data get entity @s Rotation[0]
$execute if data storage fxn:context {pos:{r1:1}} run \
    execute store result $(path).r1 int 1 run data get entity @s Rotation[1]
